...
namespace Ukoo_Circle.user_details_views
{
    ...
    public partial class SiblingsListPage : PhoneApplicationPage
    {
        ...
        private void ParseResults(string results)
        {
            if (string.IsNullOrEmpty(results))
            {
                ...
                MessageBox.Show("Fetching sibling(s) was unsuccessful...Try again");
                return;
            }
            else
            {
                ...
                {
                    List<Siblings> allrels = new List<Siblings>();
                    while (reader.Read())
                    {
        ...
        {
            NavigationService.Navigate(new Uri("/Pages/UserDetails/EditSiblingPage.xaml", UriKind.Relative));
        }
    }
}